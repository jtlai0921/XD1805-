...
Partial Class ch14_2e
    ...
    Sub Display_Student()
        ...
        '顯示資料表欄位的所有資料
        While reader.Read()
            DropDownList3.Items.Add(reader.Item("學號") & "/" & reader.Item("姓名"))
        End While
        conn.Close()
    End Sub
    ...
    Protected Sub Button16_Click(sender As Object, e As System.EventArgs) Handles Button16.Click
        Call OpenDB()  '呼叫開啟資料庫之副程式
        REM =====查詢學生選課科目清單==========
        Dim SelectCmd As String
        SelectCmd = "select 學生資料表.學號,姓名,課名,學分數 from 科系代碼資料表,學生資料表,選課資料表,課程資料表 where 學生資料表.系碼=科系代碼資料表.系碼 And 學生資料表.學號=選課資料表.學號 And 選課資料表.課號=課程資料表.課號 And 選課資料表.學號='" & Mid(DropDownList3.Text, 1, 5) & "'"
        ' SelectCmd = "select 學生資料表.學號,姓名,課名,學分數 from 科系代碼資料表,學生資料表,選課資料表,課程資料表 where 學生資料表.系碼=科系代碼資料表.系碼 And 學生資料表.學號=選課資料表.學號 And 選課資料表.課號=課程資料表.課號 And 選課資料表.學號='96001'"
        '宣告物件
        Dim DtApter As OleDbDataAdapter
        Dim DtSet As DataSet
        DtApter = New OleDbDataAdapter(SelectCmd, conn)
